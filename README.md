    ...
    └── course-name/
        ...
        └── lectures/
            └── lecture-01-topic/
                ...
                ├── lecture.pdf        # exact source-faithful LaTeX-generated PDF
                └── references.bib
templates/
└── lecture.tex
scripts/
...
- Each lecture keeps both the original handwritten source PDF and an exact, source-faithful `lecture.pdf` generated by LaTeX. The wrapper preserves every source page, including diagrams and handwritten annotations, without summarising or adding content.
- Generated LaTeX auxiliaries are excluded via `.gitignore`; source PDFs, `.tex`, figures, and bibliographies remain tracked.

## How to use the notes

Open the relevant course README to find its lecture index. Each lecture README records the date, topics, source material, and completion status. The LaTeX source is compiled with `latexmk` or `tectonic`; lecture wrappers use `pdfpages` to preserve the source page order exactly.
